...
public class PlayGenerator {
	...
	public static Play generatePlay(PlayType playType, int yardsToEndzone, Duration timeLeft) {
		int value;
		int gain = 0;
		...
		if(playType == PlayType.Passing) {
			...
			if(value < 40) {
				gain = 0;
				isIncomplete = true;
				duration = Duration.ofSeconds(random.nextInt(10)+5);
		...
		if(timeLeft.getSeconds() < Duration.ofSeconds(120).getSeconds()) {
			if(playType == PlayType.Passing && !isIncomplete) {
